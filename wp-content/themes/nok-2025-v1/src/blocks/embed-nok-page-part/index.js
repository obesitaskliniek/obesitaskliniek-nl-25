/**
 * Embed NOK Page Part Block
 *
 * Renders page part content within an iframe for the block editor preview.
 *
 * SECURITY NOTE: dangerouslySetInnerHTML Usage
 * =============================================
 * This component uses dangerouslySetInnerHTML in several places to render HTML content.
 * This is an intentional decision that has been security-reviewed:
 *
 * 1. All HTML sources are TRUSTED:
 *    - Page part titles come from WordPress post titles (admin-controlled)
 *    - Template names come from our own registry (theme-controlled)
 *    - Display labels are derived from admin-created page parts
 *
 * 2. This is admin-only UI:
 *    - The block editor is only accessible to authenticated users with edit_posts capability
 *    - Malicious content would require a compromised admin account (game over anyway)
 *
 * 3. The preview iframe provides isolation:
 *    - The actual page part HTML renders in an iframe via REST API
 *    - The iframe content is generated by TemplateRenderer using escaped field values
 *
 * 4. Alternatives considered:
 *    - DOMPurify: Would add dependency and may strip legitimate HTML/CSS
 *    - Text-only rendering: Would break the UI which relies on HTML formatting
 *
 * If you need to add user-submitted content to this component in the future,
 * you MUST sanitize it. Consider using DOMPurify:
 * ```
