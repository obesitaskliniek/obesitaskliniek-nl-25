...
import {isVisible} from "./domule/util.observe.min.mjs";
import mediaInfo from "./domule/util.mediainfo.min.mjs";
...
export function setupFakeScrollbar(scrollElement) {
    ...
    if (scrollElement.dataset.noScrollbar !== 'true') {
        // build the new nodes
      ...
      updateScrollbar(); // Initial update for the faked scollbar
    }
    ...
    if (scrollElement.dataset.autoscroll === 'true' && !mediaInfo('prefers-reduced-motion')) {
        const interval = Math.max(1000, +scrollElement.dataset.autoscrollInterval || 10000);
        const resumeDelay = scrollElement.dataset.resumeScroll ? Math.max(1000, +scrollElement.dataset.resumeScroll) : 0;
        const children = Array.from(scrollElement.children);
        let timerId = null;
        let waitTimer = null;
        ...
        function scheduleResume() {
            ...
            waitTimer = setTimeout(go, resumeDelay || interval);
        }

      (scrollbarTrack ? [scrollElement, scrollbarTrack] : [scrollElement]).forEach(el => {
            ['pointerdown', 'wheel', 'touchstart', 'mouseenter', 'keydown'].forEach(evt =>
                el.addEventListener(evt, () => {
                    clearInterval(timerId);
                    if (resumeDelay) scheduleResume();
                }, {passive: true})
            );
