 ...
 * Custom Fields:
 * - colors:select(Transparant::nok-bg-body|Grijs::nok-bg-body--darker gradient-background|Wit::nok-bg-white nok-dark-bg-darkestblue nok-text-darkblue|Blauw::nok-bg-darkerblue nok-text-contrast)!page-editable!default(Blauw)
 ...
 * - accordion_items:repeater(title:text,content:textarea)!descr[Voeg accordion items toe]
                        ...
                        foreach ( $accordion_data as $index => $item ) : ?>
                            <nok-accordion class="<?= $c->accordion_bordered->isTrue((($index !== count($accordion_data) - 1) ? 'nok-border-bottom-1' : ''), '' ); ?> <?= $c->accordion_checkmarks->is('checkmarks', 'checkmarks'); ?> <?= $c->accordion_checkmarks->is('question-marks', 'question-marks'); ?>">
                                <details
                                    ...
                                    <summary class="nok-py-1 nok-px-2 nok-fs-3 nok-fs-to-sm-2 fw-bold" style="--checkmark-color:<?= $c->accordion_checkmarks_colors ?>;">
                                        <?= esc_html( $item['title'] ) ?>
                                    </summary>
